...
def check_consistency(original_df: pd.DataFrame, generated_df: pd.DataFrame) -> pd.DataFrame:
    errors = []
    for idx, (orig, gen) in enumerate(zip(original_df.itertuples(index=False), generated_df.itertuples(index=False))):
        ...
        missing = []
        if str(getattr(orig, "Gender", "")).strip() not in sentence:
            missing.append("Gender")
        if str(getattr(orig, "Age", "")).strip() not in sentence:
            missing.append("Age")
        if str(getattr(orig, "Education", "")).strip() not in sentence:
            missing.append("Education")
        if str(getattr(orig, "Employment", "")).strip() not in sentence:
...
def main(argv=None):
    ...
    for suffix in tqdm(paths.scenarios(attr), desc="Scenarios"):
        ...
        try:
            errors_df = check_consistency(original_df, generated_df)
            if errors_df.empty:
