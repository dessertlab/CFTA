...
make_strata <- function(df) {
  df %>%
    mutate(
      X  = as.factor(Gender),
      W1 = as.factor(Education),
      W2 = as.factor(Employment),
      Zb = cut(Age, breaks = age_breaks, right = FALSE, include.lowest = TRUE)
    ) %>%
    tidyr::unite("STRATA", X, W1, W2, Zb, remove = FALSE, sep = " | ")
}
...
check_stratification <- function(df_orig, df_sub, vars = c("Gender","Education","Employment","Zb")) {
